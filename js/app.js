...
function calc(d) {
  const kMed=parseFloat(d.kostprijsPerUur)||65, kCoord=parseFloat(d.kostprijsCoordinatorPerUur)||kMed;
  const men=parseFloat(d.aantalMensenPerIncident)||0, dur=parseFloat(d.duurIncidentMinuten)||0, fr=parseFloat(d.frequentiePerWeek)||0;
  ...
  const eenmalig=parseFloat(d.eenmaligeKosten)||0, licentie=parseFloat(d.jaarlijkseLicentie)||0;
  ...
  const bespAdminJaar=(adminUur*kCoord*52)*REDUCTIE;
  const bespTijdverliesJaar=(verlorenUrenPerWeek*kMed*52)*REDUCTIE;
  const bespOmzetJaar=(nietGefMaand*12)*REDUCTIE;
  ...
  return {verlorenUrenPerWeek,tijdverliesMaand,adminMaand,nietGefMaand,totaalHuidigMaand,bespAdminJaar,bespTijdverliesJaar,bespOmzetJaar,totaleJaarlijkseBesparing,kostenJaar1,kostenJaar2,kostenJaar3,totaleKosten3Jaar,totaleBesparing3Jaar,nettoWinstJaar1,nettoWinstJaarNa1,breakEvenMaanden,roi3Jaar,cumKosten,cumBesparing,eenmalig,licentie};
}
...
function renderForm(){
  ...
  if(s===0){body=`
    ...
    <div class="field-grid">
      ...
      <div class="field"><label>Kostprijs per uur (€) ${infoTip("Tip: Gebruik hier de totale werkgeverslasten (inclusief pensioen, verzekeringen en vakantiegeld), niet alleen het brutoloon. Dit ligt vaak 1,3 tot 1,5 keer hoger.")}</label><input type="number" id="f1" value="${d.kostprijsPerUur}" placeholder="65"><p class="hint">Standaard: € 65/u</p></div>
    </div>
    ...
    <div class="field-grid">
      ...
      <div class="field"><label>Kostprijs coördinator per uur (€) ${infoTip("Tip: Gebruik hier de totale werkgeverslasten (inclusief pensioen, verzekeringen en vakantiegeld), niet alleen het brutoloon. Dit ligt vaak 1,3 tot 1,5 keer hoger.")}</label><input type="number" id="f1" value="${d.kostprijsCoordinatorPerUur||d.kostprijsPerUur}" placeholder="${d.kostprijsPerUur||65}"><p class="hint">Standaard: teamgemiddelde</p></div>
    </div>
...
function renderResult(){
  ...
  const payback=r.breakEvenMaanden;
  set("app",`<div>
    ...
    <div class="kpi-grid">
      <div class="kpi-card accent"><div class="kpi-label">Terugverdientijd</div><div class="kpi-value">${payback!=null?fmtM(payback):"—"}</div><div class="kpi-sub">${payback!=null?"Binnen "+fmtM(payback)+" terugverdiend":"Geen terugverdientijd berekend"}</div></div>
    ...
    <div class="tables-grid">
      ...
      <div class="card" style="margin:0"><span class="label">Bruto besparing / jaar (na realisme-correctie)</span>
    ...
    <div class="aannames"><strong style="color:#666">Aannames:</strong> Realismecorrectie van 20% op alle besparingen · ROI-projectie over 3 jaar · Geen interne trainingskosten · Besparingen constant over 3 jaar</div>
...
function generatePDF(){
  ...
  setTimeout(()=>{
    try{
      ...
      doc.setFont("helvetica","normal"); doc.setFontSize(8); doc.setTextColor(130,130,130); doc.text([d.sector,d.contactpersoon].filter(Boolean).join(" · "),mg,y); y+=9;
      ...
      doc.text("Huidige situatie / maand",mg,y); doc.text("Bruto besparing / jaar",mg+tw+8,y); y+=3;
      ...
      const cW=(W-mg*2)/4;
      doc.setFontSize(7); doc.setTextColor(110,110,110); doc.text("Jaar 1",mg+cW,y,{align:"right"}); doc.text("Jaar 2",mg+cW*2,y,{align:"right"}); doc.text("Jaar 3",mg+cW*3,y,{align:"right"}); y+=5;
      doc.setFontSize(8); doc.setTextColor(70,70,70); doc.text("Kosten",mg,y); doc.text(fmt(r.kostenJaar1),mg+cW,y,{align:"right"}); doc.text(fmt(r.kostenJaar2),mg+cW*2,y,{align:"right"}); doc.text(fmt(r.kostenJaar3),mg+cW*3,y,{align:"right"}); y+=6;
      doc.setTextColor(5,150,105); doc.text("Besparing",mg,y); doc.text(fmt(r.totaleJaarlijkseBesparing),mg+cW,y,{align:"right"}); doc.text(fmt(r.totaleJaarlijkseBesparing),mg+cW*2,y,{align:"right"}); doc.text(fmt(r.totaleJaarlijkseBesparing),mg+cW*3,y,{align:"right"}); y+=6;
      doc.setFont("helvetica","bold"); doc.setTextColor(91,91,214); doc.text("Netto",mg,y); doc.text(fmt(r.nettoWinstJaar1),mg+cW,y,{align:"right"}); doc.text(fmt(r.nettoWinstJaarNa1),mg+cW*2,y,{align:"right"}); doc.text(fmt(r.nettoWinstJaarNa1),mg+cW*3,y,{align:"right"}); y+=10;
      doc.setFillColor(245,245,250); doc.roundedRect(mg,y,W-mg*2,17,2,2,"F");
      doc.setFont("helvetica","bold"); doc.setFontSize(6.5); doc.setTextColor(91,91,214); doc.text("SAMENVATTING",mg+5,y+6);
      doc.setFont("helvetica","normal"); doc.setFontSize(7.5); doc.setTextColor(80,80,80); doc.text(`Ontwikkeling: ${fmt(r.eenmalig)}`,mg+5,y+12.5); doc.text(`Licentie/jaar: ${fmt(r.licentie)}`,mg+58,y+12.5);
      doc.setFont("helvetica","bold"); doc.setTextColor(91,91,214); doc.text(`ROI 3 jaar: ${r.roi3Jaar!=null?fmtPct(r.roi3Jaar):"n.v.t."}`,mg+115,y+12.5); y+=25;
      doc.setFont("helvetica","normal"); doc.setFontSize(7); doc.setTextColor(150,150,150);
      doc.text("Aannames: realismecorrectie 20% · projectie 3 jaar · geen interne trainingskosten · besparingen constant",mg,y); y+=7;
      doc.setDrawColor(210); doc.setLineWidth(0.3); doc.line(mg,y,W-mg,y); y+=5;
      doc.setFontSize(7); doc.setTextColor(180,180,180); doc.text("Gegenereerd via Aziri ROI Calculator · aziri.be",mg,y); doc.text(`ROI_${naam}_Aziri`,W-mg,y,{align:"right"});
